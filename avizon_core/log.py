from avizon_core.time import get_now_date, get_now_time
from avizon_core.network import get_size


def store_restart_avizon_uploader_log():
    file = open('/var/www/avizon/avizon.log', 'a')
    file.write(f"date: {get_now_date()} - "
               f"time: {get_now_time()} - "
               f"avizon.service restarted\n")
    file.close()
